...
POLL_INTERVAL = int(os.environ.get("BRIGHTDATA_POLL_INTERVAL", 30))
MAX_ATTEMPTS = int(os.environ.get("BRIGHTDATA_MAX_ATTEMPTS", 60))
...
def retrieve_collection(collection_id: str) -> dict:
    ...
    for _ in range(MAX_ATTEMPTS):
        ...
        try:
            ...
            if not body:
                # Bright Data can return 200 with an empty body while the
                # collection is still assembling. Treat it the same as the
                # 202 "not ready" case below, not as a hard failure.
